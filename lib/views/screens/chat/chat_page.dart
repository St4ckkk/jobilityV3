...
class _ChatPageState extends State<ChatPage> {
  ...
  String userUid = ''; // Add userUid initialization
  bool isAgent = false;
  ...
  void sendMessage() async {
    ...
    String receiverName = chat['receiverName'] ?? 'Anonymous';
  ...
  Widget build(BuildContext context) {
    ...
    if (chatRoomId.isEmpty) {
      return Scaffold(
        body: Center(
          child: Text('Chat room ID is not available'),
        ),
      );
    }

    final Stream<QuerySnapshot> typingStatus = chatRoomId.isNotEmpty
    ...
    final Stream<QuerySnapshot> chats = chatRoomId.isNotEmpty
        ? FirebaseFirestore.instance
        .collection('chats')
        .doc(chatRoomId)
        .collection('messages')
    ...
    final Stream<QuerySnapshot> userStatus = userUid.isNotEmpty
        ? FirebaseFirestore.instance
        .collection('status')
        .doc('status')
        .collection(userUid)
                      ...
                      builder: (BuildContext context, snapshot) {
                        ...
                        return Consumer<AgentNotifier>(
                          builder: (context, agentNotifier, child) {
                            ...
                            String secondUser = users
                                .firstWhere((user) => user != userUid);
